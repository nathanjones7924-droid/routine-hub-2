...
struct ActionEditorView: View {
    ...
    @State private var useLoudAlarm: Bool
    ...
    init(action: RoutineAction?, onSave: @escaping (RoutineAction) -> Void) {
        ...
        if let action = action {
            ...
            let totalSeconds = action.durationSeconds
            _durationHours = State(initialValue: totalSeconds / 3600)
            _durationMinutes = State(initialValue: (totalSeconds % 3600) / 60)
            ...
            _useLoudAlarm = State(initialValue: action.useLoudAlarm)
        } else {
            ...
            _useLoudAlarm = State(initialValue: false)
        }
    }
    ...
    private func quickDurationButton(minutes: Int, label: String) -> some View {
        Button {
            ...
            Text(label)
                .font(AppTheme.caption)
                .fontWeight(.medium)
                .foregroundColor(durationHours == 0 && durationMinutes == minutes && durationSeconds == 0 ? .white : AppTheme.primaryColor)
                .padding(.horizontal, 12)
                .padding(.vertical, 8)
                .background(durationHours == 0 && durationMinutes == minutes && durationSeconds == 0 ? AppTheme.primaryColor : AppTheme.elevatedBackground)
        ...
        VStack(alignment: .leading, spacing: AppTheme.paddingSmall) {
            ...
            VStack(spacing: 0) {
                ...
                if isAlarmEnabled {
                    ...
                    Toggle(isOn: $useLoudAlarm) {
                        HStack(spacing: 12) {
                            Image(systemName: useLoudAlarm ? "speaker.wave.3.fill" : "speaker.wave.1.fill")
                                .font(.system(size: 18))
                                .foregroundColor(useLoudAlarm ? .orange : AppTheme.secondaryText)
                            VStack(alignment: .leading, spacing: 2) {
            ...
            .animation(.easeInOut(duration: 0.25), value: useLoudAlarm)
        }
    }
    ...
    private func saveAction() {
        let totalSeconds = (durationHours * 3600) + (durationMinutes * 60) + durationSeconds
