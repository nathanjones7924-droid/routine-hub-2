...
class AppDelegate: NSObject, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, willPresent notification: UNNotification, withCompletionHandler completionHandler: @escaping (UNNotificationPresentationOptions) -> Void) {
        ...
        // Check if it's an action completion notification
        if identifier.starts(with: "action_completion_") {
            print("[AppDelegate] Action completion notification received")
            
            // Mark timer as completed and play the appropriate sound
            Task { @MainActor in
                if let timerManager = AppDelegate.timerManager {
                    let userInfo = notification.request.content.userInfo
                    let useLoudAlarm = userInfo["useLoudAlarm"] as? Bool ?? false
                    let isAlarmEnabled = userInfo["isAlarmEnabled"] as? Bool ?? true
                    
                    // Mark action as completed
                    timerManager.markCurrentActionCompleted()
                    
                    // Play sound based on action setting
                    if isAlarmEnabled {
                        timerManager.playCompletionSound(useLoud: useLoudAlarm)
                    }
                }
            }
            
            // Don't show notification when app is in foreground - just play the sound
            completionHandler([])
            return
        }
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        // Check if it's an action completion notification
        if identifier.starts(with: "action_completion_") {
            print("[AppDelegate] User tapped action completion notification")
            Task { @MainActor in
                // Sync timer state - mark as completed
                if let timerManager = AppDelegate.timerManager {
                    timerManager.markCurrentActionCompleted()
                }
            }
...
struct Morning_RoutineApp: App {
    ...
    // Track scene phase for syncing timer
        ...
        WindowGroup {
            ContentView()
                ...
                .onAppear {
                    ...
                    routineManager.locationManager = locationManager
                    routineManager.settingsManager = settingsManager
                    locationManager.settingsManager = settingsManager
                ...
                .onChange(of: routineManager.selectedRoutineIds) { _, newSelectedIds in
                    // Sync selected routines to alarm manager
                    alarmManager.selectedRoutineIds = newSelectedIds
                    Task {
                        await alarmManager.updateAllAlarms(routines: routineManager.routines)
                    }
                }
