...
struct RoutineAction: Identifiable, Codable, Equatable {
    ...
    /// Whether to use loud alarm (true) or quiet beep (false) when alarm fires
    var useLoudAlarm: Bool
    
    /// Creates a new RoutineAction
    init(id: UUID = UUID(), name: String = "", durationSeconds: Int = 60, useRedFilter: Bool = false, isAlarmEnabled: Bool = true, useLoudAlarm: Bool = false) {
        self.id = id
        self.name = name
        self.durationSeconds = durationSeconds
        self.useRedFilter = useRedFilter
        self.isAlarmEnabled = isAlarmEnabled
        self.useLoudAlarm = useLoudAlarm
    }
